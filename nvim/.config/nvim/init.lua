...
Kickstart.nvim is a template for your own configuration. The goal is
  that you can read every line of code, top-to-bottom, understand what
  your configuration is doing, and modify it to suit your needs.

  Once you've done that, you should start exploring, configuring and
  tinkering to explore Neovim!
...
I have left several `:help X` comments throughout the init.lua You
should run that command and read that help section for more information.

In addition, I have some `NOTE:` items throughout the file. These are
for you, the reader to help understand what is happening. Feel free to
delete them once you know what you're doing, but they should serve as a
guide for when you are first encountering a few different constructs in
your nvim config.
...
-- Set <space> as the leader key See `:help mapleader` NOTE: Must happen
-- before plugins are required (otherwise wrong leader will be used)
vim.g.mapleader = ' '
...
-- NOTE: Here is where you install your plugins. You can configure
-- plugins using the `config` key.
--
...
require('lazy').setup({
  ...
  --
  -- Visual Mode to select, then 'gc' to toggle.
  ...
  -- The configuration is done below. Search for lspconfig to find it
  -- below.
  {
  ...
  -- kickstart repo. Uncomment the following line and add your plugins
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  pattern = 'bash,c,cpp,lua,javascript,make,python,ruby,rust,markdown',
...
-- They will be passed to the `settings` field of the server config. You
-- must look up that documentation yourself.
--
-- If you want to override the default filetypes that your language
-- server will attach to you can define the property 'filetypes' to the
-- map in question.
local servers = {
